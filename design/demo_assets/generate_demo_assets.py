...
def generate_compare_ocr():
    ...
    draw.text(
        (70, 470), "鲜山楂、低聚果糖、浓缩苹果汁", fill=COLOR_TEXT, font=body_font
    )
...
def generate_case_clear():
    ...
    draw.text(
        (50, 30), "清晰配料表案例：沂蒙公社山楂糕", fill=COLOR_TEXT, font=title_font
    )
    ...
    draw.text(
        (70, 200), "鲜山楂、低聚果糖、浓缩苹果汁", fill=COLOR_TEXT, font=body_font
    )
...
def generate_case_blur():
    ...
    draw.text(
        (50, 30), "模糊图片失败案例：手抖/光线不足", fill=COLOR_TEXT, font=title_font
    )
