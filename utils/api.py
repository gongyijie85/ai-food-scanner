...
def build_system_prompt(groups):
    ...
    return (
        ...
        "- ocr_text: 配料表区域图片中实际出现的**完整原始文字**（含标点、括号、添加量等），不得补全、不得推断、不得省略\n"
        "- ingredients: 所有配料成分列表，**必须严格从 ocr_text 提取**，顺序与 ocr_text 一致。禁止把基础配料（水、饮用水、白砂糖、白糖、冰糖、红糖、食用盐、食盐、食用油、植物油、面粉、大米、淀粉、食品用香精、食用香精、香精、酵母、蜂蜜、麦芽糖浆、果葡糖浆、葡萄糖浆）列入 additives。\n"
        "- additives: 只含 GB 2760 具体食品添加剂。**绝对不要**把基础配料、保健品辅料列入。每个添加剂必须含 name（字符串），可选 code（INS/E号，没有留空）。additives 必须是数组，无添加剂时传 []。**不要输出 level 字段，不要给 score 评分，风险等级由系统判定。**\n"
        "- advice: 针对以下人群的一句话建议，使用固定句式：" + group_str + "。"
        "例如：普通人群可适量食用，建议保持均衡饮食；糖尿病患者请注意控制摄入量，具体请咨询医生或营养师。"
        "只输出一句，禁止医学疗效词。\n\n"
        "## 强制规则（两类产品都适用）\n"
        "- product_name **必须中文**，英文产品名翻译成中文或填'该产品'\n"
        "- **先完整 OCR 配料表原文到 ocr_text，再从 ocr_text 提取 ingredients/additives；ingredients 中的每一项必须能在 ocr_text 中找到对应文字**\n"
        "- **禁止根据产品类型、常识、宣传文案、营养成分表或'常见配方'推测配料**；禁止把'可能含有'当作配料返回；禁止把 ocr_text 中没有的文字加入 ingredients\n"
        ...
        "### 普通食品示例\n"
        '{"type":"food","product_name":"某牌苏打饼干",'
        '"ocr_text":"配料：小麦粉、植物油、食用盐、碳酸氢钠、酵母。",'
        '"ingredients":["小麦粉","植物油","食用盐","碳酸氢钠","酵母"],'
        ...
        "messages": [
            {"role": "system", "content": system_prompt},
            {
                ...
                "content": [
                    ...
                    {
                        "type": "text",
                        "text": "请严格根据图片中配料表实际出现的文字回答。第一步先输出配料表区域的完整原始文字到 ocr_text，第二步只从 ocr_text 中提取 ingredients/additives，禁止加入 ocr_text 中没有显示的配料。按规则返回 JSON。",
                    },
                ],
            },
        ],
