...
### 用户反馈：待确认 / 分数动画 / 自然语音 / 使用指引

- **待确认误标**：`parse_result` 写入完整 `status=rated` 等字段；山梨酸钾等有风险级时显示「注意」而非「待确认」
- **分数滚动**：结果卡片参考分 0→目标分缓动动画
- **更自然语音**：优先 `edge-tts` 晓晓神经网络音色生成 MP3，失败再回退浏览器 TTS
- **使用指引**：首页/扫描/结果页首次可展开三步说明与拍照技巧
- **依赖**：`requirements.txt` 增加 `edge-tts`
- **测试**：`pytest` 137 项通过
...
- 根因：TTS 在 iframe 里绑定父页面按钮，手机/微信丢失用户手势，`speak` 被静默拦截；unlock 后立刻 `cancel` 也会清队列
- 修复：听结果按钮与 `speechSynthesis` 同组件同点击回调；文案 base64 传参；去掉错误 unlock
- 微信内打开给出「请用浏览器打开」提示
...
- **短产品名**：去掉前导英文品牌与品类括号（如 PEI TIEN…(膨化食品)）
- **状态一致**：有 B 级添加剂时显示「有可留意项」，不再高分却写「暂未发现明显问题」
- **友好时间**：`2026-07-27T01:27:46` → `2026年7月27日 01:27`
- **详情页听结果**：历史回看可语音播报；「图片未保存」改为「本次未保留包装照片」
- **测试**：`utils/display.py` + `pytest` 108 项通过

## v0.10.20 - 2026-07-25

### 针对「芝士鳕鱼肠」类标签识别偏差

- **根因**：GB2760 SQLite 仅 294 条，常见添加剂（山梨酸钾、三聚磷酸钠等）只在 CSV 风险表；匹配器未回退 CSV → 显示「未识别到」且 100 分虚高
- **修复**：`AdditiveMatcher` 标准库未命中时回退 CSV 风险表
- **补捞**：从 OCR/复合配料括号中补捞添加剂，长名优先占用，避免「山梨酸」截胡「山梨酸钾」
- **配料切分**：尊重嵌套括号，避免鱼糜(...) 被拆碎
- **文案**：「未识别到」改为「待核对包装」
- **测试**：`pytest` 101 项通过
...
- **配料空列表**：`utils/api.py` 在 `ingredients` 为空但有 `ocr_text` 时自动切分恢复；结果页始终展示配料区与原文/重拍提示
- **手机语音**：长文本分句排队、移动端中文声优优先、手势 unlock 与更明确失败提示
- **手机 UI**：扫描提示与更大上传区、全宽「听结果」、配料标签与底栏堆叠、安全区留白
- **结果页**：语音按钮上移；播报文案含关注提示与配料/添加剂摘要
- **测试**：`pytest` 99 项通过
...
- **文件**：论坛帖子 https://forum.trae.cn/t/topic/51391
- **调整头部信息顺序**：将报名帖链接从第 3 项移到第 5 项（公开体验之后、代码仓库之前），并标注「已通过审核」。
- **更新版本号**：正文「当前版本」由 v0.9.3 改为 v0.10.18（初赛最终提交版本）。
- **补充更新说明**：在「最新更新」章节新增 v0.10.13–v0.10.17 四段变更摘要。
- **修复 Session ID**：7/13 行重复 ID 由 `6a4f309cea3ecc845cacdb16` 改为 `6a56232a44e13ac775c44030`。
- **验证结果**：公开体验链接 HTTP 200，帖子关键信息已通过 no-cache 请求逐项核对。

## v0.10.17 - 2026-07-15

### AI 食品配料表识别工具 v0.10.17（热修复扫描后结果页报错）

- **文件**：`components/score_hero.py`、`app.py`、`README.md`
- **修复 `TypeError`**：`components/score_hero.py` 中 `st.markdown()` 被误传 `key` 参数导致扫描后结果页报错，已移除该非法参数，结果页可正常渲染。
- **版本号同步**：`app.py` 与 `README.md` 版本徽章同步升级到 v0.10.17。
- **全量质量门禁结果**：`python -m pytest -q` 97 项通过；`python -m compileall -q .` 通过。
